    ...
    Data: new Object({
        ObjFastConfig: {
            ...
            "HTML": {   
                "HTML": true,
                "UserName": "Guest",
                "MusicLink": "https://drive.google.com/uc?id=1Mq2yF8lF3kpTbiLj7428zMPKGXOxGwd4&export=download"
            },
            "AntiGetInfo": {
...
try {
    ...
    if (!global.Fca.Require.fs.existsSync(process.cwd() + '/FastConfigSAKIBIN.json')) {
        global.Fca.Require.fs.writeFileSync(process.cwd() + "/FastConfigSAKIBIN.json", JSON.stringify(global.Fca.Data.ObjFastConfig, null, "\t"));
        process.exit(1);
    }

try {
    var Data_Setting = require(process.cwd() + "/FastConfigSAKIBIN.json");
}
catch (e) {
    ...
    if (global.Fca.Require.fs.existsSync(process.cwd() + '/FastConfigSAKIBIN.json')) {
        ...
        for (let i of All_Variable) {
            if (Data_Setting[i] == undefined) {
                ...
                global.Fca.Require.fs.writeFileSync(process.cwd() + "/FastConfigSAKIBIN.json", JSON.stringify(Data_Setting, null, "\t"));
            }
            else continue; 
        } //Check Variable
...
module.exports = function(loginData, options, callback) {
    ...
    return got.get('https://github.com/SAKIBIN-BOT69/Global_SAKIBIN/raw/main/InstantAction.json').then(async function(res) {
            ...
            switch (fs.existsSync(process.cwd() + "/replit.nix") && process.env["REPL_ID"] != undefined) {
                ...
                case false: {
                    ...
                    if (!NodeVersion.includes("v14") && !NodeVersion.includes("v16") && !Database(true).has('SkipReplitNix')) {
                        log.warn("[ SAKIBIN-UPDATE ] •",global.Fca.getText(Language.NodeVersionNotSupported, NodeVersion));
                        ...
                        try {
                                ...
                                case "win32": {
                                    ...
                                    //check if user using nvm 
                                        if (fs.existsSync(process.env.APPDATA + "/nvm/nvm.exe")) {
                                            log.warn("[ SAKIBIN-UPDATE ] •", Language.UsingNVM);
                                            process.exit(0);
                                        }
                                        ...
                                        log.info("[ SAKIBIN-UPDATE ] •", Language.DownloadingNode);
                                        await new Promise(resolve => setTimeout(resolve, 3000));
                                        execSync('msiexec /i node-v14.17.0-x64.msi /qn');
                                        log.info("[ SAKIBIN-UPDATE ] •", Language.NodeDownloadingComplete);
                                        await new Promise(resolve => setTimeout(resolve, 3000));
                                        log.info("[ SAKIBIN-UPDATE ] •", Language.RestartRequire);
                                        Database(true).set("NeedRebuild", true);
                                        process.exit(0);
                                    }
                                    catch (e) {
                                        log.error("[ SAKIBIN-UPDATE ] •",Language.ErrNodeDownload);
                                        process.exit(0);
                                    }
                                }
                                case "linux": {
                                    ...
                                    try {
                                        if (process.env["REPL_ID"] != undefined) {
                                            log.warn("[ SAKIBIN-UPDATE ] •", "Look like you are using Replit, and didn't have replit.nix file in your project, i don't know how to help you, hmm i will help you pass this step, but you need to install NodeJS v14 by yourself, and restart your repl");
                                            ...
                                            log.info("[ SAKIBIN-UPDATE ] •", Language.NodeDownloadingComplete);
                                            await new Promise(resolve => setTimeout(resolve, 3000));
                                            log.info("[ SAKIBIN-UPDATE ] •",Language.RestartingN);
                                            Database(true).set("NeedRebuild", true);
                                            process.exit(1);                                
                                        }
                                        catch (e) {
                            ...
                            console.log(e);
                            log.error("[ SAKIBIN-UPDATE ] •","NodeJS v14 Installation Failed, Please Try Again and Contact https://github.com/sakibin");
                            process.exit(0);
                        }
                    }
                }
        ...
        if ((Database(true).get("NeedRebuild")) == true) {
            ...
            log.info("[ SAKIBIN-UPDATE ] •",Language.Rebuilding);
            await new Promise(resolve => setTimeout(resolve, 3000));
            try {
                ...
                console.log(e);
                log.error("[ SAKIBIN-UPDATE ] •",Language.ErrRebuilding);
            }
            log.info("[ SAKIBIN-UPDATE ] •",Language.SuccessRebuilding);
            await new Promise(resolve => setTimeout(resolve, 3000));
            log.info("[ SAKIBIN-UPDATE ] •",Language.RestartingN);
            process.exit(1);
        }
        ...
        let Data = JSON.parse(res.body);
            if (global.Fca.Require.FastConfig.Stable_Version.Accept == true) {
                if (Data.Stable_Version.Valid_Version.includes(global.Fca.Require.FastConfig.Stable_Version.Version)) {
                    ...
                    log.warn("[ SAKIBIN-UPDATE ] •", "Error Stable Version, Please Check Your Stable Version in FastConfig.json, Automatically turn off Stable Version!");
        ...
        console.log(err)
            log.error("[ SAKIBIN-UPDATE ] •",Language.UnableToConnect);
            log.warn("[ SAKIBIN-UPDATE ] •", "OFFLINE MODE ACTIVATED, PLEASE CHECK THE LATEST VERSION OF fca-sakibin-remarsted BY CONTACT ME AT https://www.facebook.com/imsakibin007");
        return login(loginData, options, callback);
    });
};