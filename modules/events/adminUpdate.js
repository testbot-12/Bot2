    ...
    try {
        ...
        switch (logMessageType) {
            ...
            case "log:thread-admins": {
                if (logMessageData.ADMIN_EVENT == "add_admin") {
                    dataThread.adminIDs.push({ id: logMessageData.TARGET_ID })
                    api.sendMessage(`[ New Activity ]✅\n» 𝙉𝙚𝙬 𝘼𝙙𝙢𝙞𝙣 \n\n ${logMessageData.TARGET_ID}  সে এখন Group Admin`, threadID);
                }
                else if (logMessageData.ADMIN_EVENT == "remove_admin") {
                    dataThread.adminIDs.nicknames = dataThread.adminIDs.nicknames.filter(item => item.adminIDs.nicknames != logMessageData.adminIDs.nicknames);
                    api.sendMessage(`[ New Activity ]✅\n» Admin থেকে remove করা হলো ${logMessageData.nicknames} `, threadID);
                }
                break;
            }
